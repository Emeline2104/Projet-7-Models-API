...
Ce module propose des fonctions pour entraîner et évaluer des modèles de classification, 
y compris des modèles de régression logistique, Light GBM, Random Forest, et un Dummy Classifier en baseline.
...
Fonctions:
    ...
    - reg_log(df, balance=None): Réalise la régression logistique en utilisant 
un ensemble de données et effectue l'optimisation des hyperparamètres.
    - kfold_lightgbm(df, balance=None): Réalise la régression light GBM 
en utilisant un ensemble de données et effectue l'optimisation des hyperparamètres.
    - random_forest(df, balance=None): Réalise la régression Random Forest en utilisant 
un ensemble de données et effectue l'optimisation des hyperparamètres.
...
def dummy_classifier(df, balance=None):
    ...
    best_model = train_and_evaluate_model(train_x,
                                          train_y,
                                          test_x,
                                          test_y,
                                          model_selec,
...
def reg_log(df, balance=None):
    """
    Réalise la régression logistique en utilisant un ensemble de données et 
...
def kfold_lightgbm(df, balance=None):
    """
    Réalise la régression light GBM en utilisant un ensemble de données 
    et effectue l'optimisation des hyperparamètres.
...
def random_forest(df, balance=None):
    """
    Réalise la régression Random Forest en utilisant un ensemble de données 
    et effectue l'optimisation des hyperparamètres.
